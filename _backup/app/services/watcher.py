...
class PDFHandler(FileSystemEventHandler):
    ...
    def process_file(self, file_path):
        parsed_result = parse_pdf(file_path=file_path)
        if parsed_result and parsed_result.get('tasks'):
            with app.app_context():
                for parsed_data in parsed_result['tasks']:
                    # Проверка на дубликаты
                    existing_task = Task.query.filter_by(file_hash=parsed_data['file_hash'], item_number=parsed_data['title']).first()
                    if existing_task:
                    ...
                    new_task = Task(
                        item_number=parsed_data.get('item_number', 'Б/Н'),
